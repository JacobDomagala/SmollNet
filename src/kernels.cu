...
__global__ void k_sum_dim0(const float* in, float* out,
                           int64_t d0, int64_t rest)
{
  ...
  for (int64_t i = 0; i < d0; ++i, p += rest) acc += *p;
  out[idx] = acc;
}

void launch_sum_dim0(void* out, void* in,
                     int64_t d0, int64_t rest)
{
  int64_t n = rest;
  int block = 256;
  int grid  = (n + block - 1) / block;
  k_sum_dim0<<<grid, block>>>(static_cast<const float*>(in), static_cast<float*>(out), d0, rest);
}


__global__ void k_sum_dim1(const float* in, float* out,
                           int64_t d0, int64_t rest)
{
  int64_t idx = blockIdx.x*blockDim.x + threadIdx.x;
  if (idx >= rest) return;
...
void launch_sum_dim1(void* out, void* in, int64_t d0, int64_t d1, int64_t d2) {
  ...
  int block = 256;
  int grid  = (n + block - 1) / block;
  k_sum_dim0<<<grid, block>>>(static_cast<const float*>(in), static_cast<float*>(out), d0, n);
}
